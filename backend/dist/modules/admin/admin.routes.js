...
const tsToTZ = (col) => `(${col} AT TIME ZONE 'UTC' AT TIME ZONE '${TZ}')`;
// For TIMESTAMPTZ expressions (NOW(), generate_series output)
const tstzToTZ = (expr) => `(${expr} AT TIME ZONE '${TZ}')`;
...
router.get("/stats", async (req, res) => {
    try {
        ...
        const [usersR, ordersR, revenueR, chartR, statusR, rtAggR, rtChartR, rtTopR] = await Promise.all([
            db_1.default.query(`SELECT COUNT(*) FROM "User" WHERE role = 'USER'`),
            db_1.default.query(`SELECT COUNT(*), status FROM "Order" o WHERE 1=1 ${cfg.where} GROUP BY status`),
            db_1.default.query(`
          SELECT
          ...
          WHERE o.status NOT IN ('CANCELLED','REFUNDED') ${cfg.where}`),
            db_1.default.query(cfg.chartSql),
            db_1.default.query(`SELECT o.status, COUNT(*) AS count FROM "Order" o ${cfg.statusSql} GROUP BY o.status`),
        ...
        res.json({
            ...
            revenueChart: chartR.rows.map((r) => ({ period: r.period, revenue: Number(r.revenue), orders: Number(r.orders) })),
            ...
            realtime: {
                ...
                chart: rtChartR.rows.map((r) => ({
                    period: r.period,
                    orders: Number(r.orders),
                    revenue: Number(r.revenue),
                })),
                topProducts: rtTopR.rows.map((r) => ({
                    name: r.product_name,
...
router.get("/products-analytics", async (req, res) => {
    try {
        ...
        const prev = days * 2;
        const [flavorsR, summaryR, timeChartR, categoryR, insightsR, flavorDeclR, catLowR, funnelR] = await Promise.all([
          ...
          SELECT
            ${cfg.labelHint === "hour"
                ...
                : cfg.labelHint === "month"
                    ...
                    : `TO_CHAR(DATE(${tsToTZ('o."createdAt"')}), 'YYYY-MM-DD')`} AS period,
            COUNT(DISTINCT o.id) AS orders,
        ...
        const flavors = flavorsR.rows.map((r) => {
        ...
        const funnelMap = {};
        for (const row of funnelR.rows)
            funnelMap[row.eventType] = Number(row.cnt);
        const funnel = {
        ...
        res.json({
            ...
            timeChart: timeChartR.rows.map((r) => ({ period: r.period, orders: Number(r.orders), items: Number(r.items) })),
            categoryChart: categoryR.rows.map((r) => ({ category: r.category, revenue: Number(r.revenue), purchases: Number(r.purchases) })),
            insights,
            funnel,
            labelHint: cfg.labelHint,
        });
...
router.get("/settings", async (_req, res) => {
    try {
        const r = await db_1.default.query(`SELECT key, value FROM "AppSetting" ORDER BY key`);
        const s = {};
...
router.patch("/settings", async (req, res) => {
    try {
        ...
        for (const [k, v] of Object.entries(settings)) {
            await db_1.default.query(`INSERT INTO "AppSetting"(key,value,"updatedAt") VALUES($1,$2,NOW())
         ON CONFLICT(key) DO UPDATE SET value=$2,"updatedAt"=NOW()`, [k, String(v)]);
        }
...
router.get("/promo-codes", async (_req, res) => {
    try {
        res.json((await db_1.default.query(`SELECT * FROM "PromoCode" ORDER BY "createdAt" DESC`)).rows);
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
...
router.post("/promo-codes", async (req, res) => {
    try {
        ...
        const r = await db_1.default.query(`INSERT INTO "PromoCode"(id,code,"discountType","discountValue","minOrderAmount","usageLimit","isActive","expiresAt")
       VALUES(gen_random_uuid(),$1,$2,$3,$4,$5,$6,$7) RETURNING *`, [code.toUpperCase(), discountType, Number(discountValue),
...
router.patch("/promo-codes/:id", async (req, res) => {
    try {
        ...
        res.json((await db_1.default.query(`UPDATE "PromoCode" SET ${fields.join(",")}, "updatedAt"=NOW() WHERE id=$${i} RETURNING *`, vals)).rows[0]);
    }
    catch (e) {
        res.status(400).json({ error: e.message });
    }
...
router.delete("/promo-codes/:id", async (req, res) => {
    try {
        await db_1.default.query(`DELETE FROM "PromoCode" WHERE id=$1`, [req.params.id]);
        res.json({ success: true });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
router.post("/notify", async (req, res) => {
    try {
        ...
        const ids = userIds?.length
            ? userIds
            : (await db_1.default.query(`SELECT id FROM "User" WHERE "isBlocked"=false`)).rows.map((r) => r.id);
        if (!ids.length) {
            res.json({ sent: 0 });
            return;
        }
        const vals = ids.map((_, j) => `(gen_random_uuid(),$${j * 3 + 1},$${j * 3 + 2},$${j * 3 + 3},'${nt}',false,NOW(),NOW())`).join(",");
        const params = [];
        for (const id of ids)
            params.push(id, title.trim(), message.trim());
        await db_1.default.query(`INSERT INTO "Notification"(id,"userId",title,message,type,"isRead","createdAt","updatedAt") VALUES ${vals}`, params);
