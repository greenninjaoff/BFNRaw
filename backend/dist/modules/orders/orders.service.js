...
async function getSetting(key, fallback) {
    try {
        const r = await db_1.default.query(`SELECT value FROM "AppSetting" WHERE key = $1`, [key]);
...
async function getByUser(userId) {
    const r = await db_1.default.query(`SELECT o.id, o."userId", o.status, o."deliveryType", o."paymentMethod",
     ...
     WHERE o."userId" = $1
     GROUP BY o.id ORDER BY o."createdAt" DESC`, [userId]);
    return r.rows;
}
...
async function getOne(id) {
    const r = await db_1.default.query(`SELECT o.id, o."userId", o.status, o."deliveryType", o."paymentMethod",
     ...
     JOIN "User" u ON u.id = o."userId"
     WHERE o.id = $1
     GROUP BY o.id, u.phone, u."fullName"`, [id]);
...
async function createOrder(userId, data) {
    ...
    for (const item of items) {
        const flavorR = await db_1.default.query(`SELECT f.*, p.name as "productName"
       FROM "ProductFlavor" f JOIN "Product" p ON p.id = f."productId"
       WHERE (f.id = $1 OR f.sku = $1) AND f."isActive" = true AND f."deletedAt" IS NULL`, [item.flavorId]);
        const flavor = flavorR.rows[0];
        if (!flavor)
            throw new Error(`Product variant not found: ${item.flavorId}`);
        if (flavor.stock < item.quantity)
            throw new Error(`Insufficient stock for ${flavor.flavorName || flavor.flavorKey}`);
    ...
    let promoRow = null;
    if (data.promoCode) {
        const pr = await db_1.default.query(`SELECT * FROM "PromoCode" WHERE code = $1 AND "isActive" = true`, [String(data.promoCode).toUpperCase().trim()]);
        promoRow = pr.rows[0];
        if (promoRow) {
    ...
    let addrData = {};
    if (data.addressId) {
        const addrR = await db_1.default.query(`SELECT * FROM "Address" WHERE id = $1`, [data.addressId]);
        if (addrR.rows[0])
            addrData = addrR.rows[0];
    }
    const orderR = await db_1.default.query(`INSERT INTO "Order" (id, "userId", status, "deliveryType", "paymentMethod",
    ...
    for (const item of enrichedItems) {
        await db_1.default.query(`INSERT INTO "OrderItem" (id, "orderId", "flavorId", quantity, "unitPrice",
        "skuSnapshot", "nameSnapshot", "flavorSnapshot", "netWeightSnapshot", "imageSnapshot", "createdAt")
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())`, [order.id, item.flavorDbId, item.quantity, item.unitPrice,
        ...
        await db_1.default.query(`UPDATE "ProductFlavor" SET stock = stock - $1, "updatedAt" = NOW() WHERE id = $2`, [item.quantity, item.flavorDbId]);
    }
    // Increment promo usage count after successful order
    if (promoRow) {
        await db_1.default.query(`UPDATE "PromoCode" SET "usageCount" = "usageCount" + 1, "updatedAt" = NOW() WHERE id = $1`, [promoRow.id]).catch(() => { }); // non-critical
    }
...
async function updateStatus(id, status) {
    ...
    const current = await db_1.default.query(`SELECT * FROM "Order" WHERE id = $1`, [id]);
    if (!current.rows[0])
    ...
    if (stockRestoringStatuses.includes(upperStatus) && !alreadyRestored.includes(currentStatus)) {
        const items = await db_1.default.query(`SELECT * FROM "OrderItem" WHERE "orderId" = $1`, [id]);
        for (const item of items.rows) {
            await db_1.default.query(`UPDATE "ProductFlavor" SET stock = stock + $1, "updatedAt" = NOW() WHERE id = $2`, [item.quantity, item.flavorId]);
        }
    }
    const r = await db_1.default.query(`UPDATE "Order" SET status = $1, "updatedAt" = NOW() WHERE id = $2 RETURNING *`, [upperStatus, id]);
    return r.rows[0];
}
async function cancelOrder(id, userId) {
    const r = await db_1.default.query(`SELECT * FROM "Order" WHERE id = $1 AND "userId" = $2`, [id, userId]);
    ...
    const updated = await db_1.default.query(`UPDATE "Order" SET status = 'CANCELLED', "updatedAt" = NOW() WHERE id = $1 RETURNING *`, [id]);
