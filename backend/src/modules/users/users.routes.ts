...
router.get("/", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const vals: any[] = [];
    if (search) { where += ` AND (phone ILIKE $1 OR "fullName" ILIKE $1)`; vals.push(`%${search}%`); }
    vals.push(limit, offset);
    const r = await db.query(
      `SELECT id, phone, "fullName", role, "isBlocked", "isPhoneVerified", "createdAt"
       FROM "User" ${where} ORDER BY "createdAt" DESC
       LIMIT $${vals.length - 1} OFFSET $${vals.length}`,
      vals
    );
    const total = await db.query(`SELECT COUNT(*) FROM "User" WHERE role != 'SUPERADMIN'`);
...
router.get("/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const userR = await db.query(
      `SELECT id, phone, "fullName", role, "isBlocked", "isPhoneVerified", "createdAt", "updatedAt"
       FROM "User" WHERE id = $1`,
      [id]
    );
    if (!userR.rows[0]) { res.status(404).json({ error: "User not found" }); return; }
    ...
    const statsR = await db.query(
      `SELECT COUNT(*) as "orderCount",
       ...
       FROM "Order"
       WHERE "userId" = $1 AND status NOT IN ('CANCELLED', 'REFUNDED')`,
      [id]
    );

    // Recent orders (last 10) with items
    const ordersR = await db.query(
      `SELECT o.id, o.status, o."deliveryType", o."paymentMethod",
    ...
    // Saved addresses
    const addressesR = await db.query(
      `SELECT id, title, city, district, street, house, apartment, landmark,
    ...
    const cardsR = await db.query(
      `SELECT id, "maskedNumber", brand, "createdAt"
       FROM "SavedCard" WHERE "userId" = $1 ORDER BY "createdAt" ASC`,
      [id]
    );
...
router.patch("/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (fullName !== undefined) { fields.push(`"fullName" = $${i++}`); vals.push(fullName); }
    if (phone !== undefined) { fields.push(`phone = $${i++}`); vals.push(phone); }
    if (role !== undefined) { fields.push(`role = $${i++}`); vals.push(role); }
    if (isBlocked !== undefined) { fields.push(`"isBlocked" = $${i++}`); vals.push(isBlocked); }
    if (password) {
      const h = await bcrypt.hash(password, 10);
      fields.push(`password = $${i++}`); vals.push(h);
    }
    ...
    const r = await db.query(
      `UPDATE "User" SET ${fields.join(", ")}, "updatedAt" = NOW()
       WHERE id = $${i}
       RETURNING id, phone, "fullName", role, "isBlocked", "isPhoneVerified", "createdAt", "updatedAt"`,
      vals
    );
    res.json(r.rows[0]);
...
// ─── BLOCK USER ───────────────────────────────────────────────────────────────
router.patch("/:id/block", async (req: AuthRequest, res: Response) => {
  try {
    const r = await db.query(
      `UPDATE "User" SET "isBlocked" = $1, "updatedAt" = NOW()
       WHERE id = $2 RETURNING id, phone, "fullName", role, "isBlocked"`,
